...
## ☁️ Running in the Cloud (Google Colab)
BeeConsensus is optimized for cloud GPUs. Simply upload `beeconsensus.py` and `evaluate.py` to a T4 GPU notebook and run:
```bash
pip install transformers bitsandbytes accelerate
python evaluate.py --model_id "meta-llama/Llama-3.1-8B-Instruct" --limit 100
```
